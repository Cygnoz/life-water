...
const ViewCustomers: React.FC = () => {
  ...
  // Filter customers based on search term for first and last names
  const filteredCustomers = customers.filter((customer) =>
    `${customer.firstName} ${customer.lastName}`.toLowerCase().includes(searchTerm.toLowerCase())
  );
  
  const customersWithDueAmount = customers.map((customer) => {
    const totalRate = customer.bottleStock * customer.ratePerBottle;
    const dueAmount = customer.depositAmount - totalRate;
    return dueAmount
    
  });

  console.log(customersWithDueAmount);
            ...
            <div className="flex-1 flex gap-4">
              ...
              <div>
                ...
                <div className="text-sm text-gray-600">
                  {customersWithDueAmount.dueAmount}</div>
